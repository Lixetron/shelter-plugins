...
store.aspectRatio ??= '16 / 9' // set default;
function onStreamQualityChange() {
    ...
    if (streamConnection) {
        ...
        streamConnection.videoStreamParameters[0].maxResolution.width = Math.round(store.resolution * eval(store.aspectRatio));
        ...
        log(`Patched current user stream with resolution: ${store.resolution}, aspect ratio: ${store.aspectRatio} and fps: ${store.fps}`);
    }
}
